...
let rawContraption = File.ReadAllLines("../day-16-part-01/contraption.txt")
...
let rec driveBeams (map: Cell array2d) (beams: (int * int * Direction) list) = // (x, y, direction)
...
let energizedCount (map: Cell array2d) =
    map
...
let maxEnergizedCount =
    let northStartingPoints = [0..sizeX-1] |> List.map (fun x -> (x, 0, Direction.South))
    let westStartingPoints = [0..sizeY-1] |> List.map (fun x -> (0, x, Direction.East))
    let southStartingPoints = [0..sizeX-1] |> List.map (fun x -> (x, sizeY-1, Direction.North))
    let eastStartingPoints = [0..sizeY-1] |> List.map (fun x -> (sizeX-1, x, Direction.West))

    let allStartingPoints = List.concat [northStartingPoints; westStartingPoints; southStartingPoints; eastStartingPoints]

    allStartingPoints
        |> List.map (
            fun startingPoint ->
                let selectCell x y =
                    let item = contraptionMap[x, y]

                    { CellType = item.CellType; Beams = (item.Beams |> Array.copy) }
                let thisMap = Array2D.init sizeX sizeY selectCell

                driveBeams thisMap [startingPoint]
                thisMap |> energizedCount)
        |> List.max

printfn "%A" maxEnergizedCount
